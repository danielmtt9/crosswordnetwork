/**
 * Room activity indicators component
 */
...
interface RoomActivityIndicatorsProps {
  roomId: string;
  className?: string;
}

export function RoomActivityIndicators({ roomId, className }: RoomActivityIndicatorsProps) {
  const {
    activity,
    isLoading,
    error,
  ...
  const activityLevel = getActivityLevel(activity.activityLevel);
  const ActivityIcon = activityLevel.icon;

  const connectionStatus = getConnectionStatus(activity.isConnected);
    ...
    <Card className={className}>
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Badge variant={activity.isConnected ? 'default' : 'destructive'}>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Progress value={activity.activityLevel * 100} className="h-2" />
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="text-center p-3 bg-muted/50 rounded-lg">
            ...
            <div className="text-2xl font-bold text-green-600">
              {activity.messagesPerMinute}
            </div>
            <div className="text-xs text-muted-foreground">
              {activity.totalMessages} total
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="text-center p-2 bg-muted/50 rounded">
              <div className="text-lg font-bold text-purple-600">
                {activity.puzzleActionsPerMinute}
              </div>
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-1 max-h-32 overflow-y-auto">
            {activity.recentActivity.map((activityItem, index) => (
              <div key={index} className="flex items-center gap-2 text-xs p-2 bg-muted/50 rounded">
                <div className="flex items-center gap-1">
                  {activityItem.type === 'message' && <MessageSquare className="h-3 w-3 text-blue-500" />}
                  {activityItem.type === 'puzzle' && <Puzzle className="h-3 w-3 text-green-500" />}
                  {activityItem.type === 'user' && <Users className="h-3 w-3 text-purple-500" />}
                  <span>{activityItem.description}</span>
                </div>
                <span className="text-muted-foreground ml-auto">
                  {formatLastActivity(activityItem.timestamp)}
                </span>
              </div>
            ))}
          </div>
...
export default RoomActivityIndicators;